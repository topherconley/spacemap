...
The igraph object `ig` has a very convenient sparse representation, with `ig` occupying `r round(object.size(adjnet) / object.size(ig),0)` times smaller memory than `adjnet`.  Many powerful network analysis tools are available in the igraph R package that enhance the visualization. For example, we identify modules of the spaceMap network with igraph's edge-betweenness algorithm. Each module discovered will be colored differently when we visualize the network. 
